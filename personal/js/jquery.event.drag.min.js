        ...
        c = h.drag = {
            ...
            setup: function() {
                if (!f.data(this, c.datakey)) {
                    ...
                    i.add(this, c.mouseDown, c.init, b);
            ...
            teardown: function() {
                if (!f.data(this, c.datakey).related) {
                    ...
                    i.remove(this, c.mouseDown, c.init);
            ...
            init: function(b) {
                if("ontouchstart" in window) {
                    g.mouseDown='ontouchstart';
                    g.mouseMove='ontouchmove';
                    g.mouseUp='ontouchend';
                    
                }
                var data = b.data,
                    d;
                if (!(data.which > 0 && b.which != data.which)) if (!f(b.target).is(data.not)) if (!(data.handle && !f(b.target).closest(data.handle, b.currentTarget).length)) {
                    data.propagates = 1;
                    data.interactions = [c.interaction(this, data)];
                    data.target = b.target;
                    data.pageX = b.pageX;
                    data.pageY = b.pageY;
                    data.dragging = null;
                    d = c.hijack(b, "draginit", data);
                    if (data.propagates) {
                        if ((d = c.flatten(d)) && d.length) {
                            data.interactions = [];
                            f.each(d, function() {
                                data.interactions.push(c.interaction(this, data))
                            })
                        }
                        data.propagates = data.interactions.length;
                        data.drop !== false && h.drop && h.drop.handler(b, data);
                        c.textselect(false);
                        i.add(document, c.mouseMove + " " + c.mouseUp, c.handler, data);
                        return false
                    }
                }
            },
            ...
            handler: function(b) {
                var _data = b.data;
                switch (b.type) {
                case !_data.dragging && c.mouseMove:
                    if (Math.pow(b.pageX - _data.pageX, 2) + Math.pow(b.pageY - _data.pageY, 2) < Math.pow(_data.distance, 2)) break;
                    b.target = _data.target;
                    c.hijack(b, "dragstart", _data);
                    if (_data.propagates) _data.dragging = true;
                case c.mouseMove:
                    if (_data.dragging) {
                        c.hijack(b, "drag", _data);
                        if (_data.propagates) {
                            _data.drop !== false && h.drop && h.drop.handler(b, _data);
                            break
                        }
                        b.type = c.mouseUp
                    }
                case c.mouseUp:
                    i.remove(document, c.mouseMove + " " + c.mouseUp, c.handler);
                    if (_data.dragging) {
                        _data.drop !== false && h.drop && h.drop.handler(b, _data);
                        c.hijack(b, "dragend", _data)
                    }
                    c.textselect(true);
                    if (_data.click === false && _data.dragging) {
